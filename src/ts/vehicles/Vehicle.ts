...
export abstract class Vehicle extends THREE.Object3D implements IWorldEntity
{
	...
	{	
		if ( enableVehicleUpdate && this.sid !== undefined && this.sid !== mySid ) {
			let newPosition = this.interpolation(this.position, this.detectPosition, timeStamp / 1000);
			...
			this.quaternion.slerp(newQuaternion, 0.08);

			this.collision.interpolatedQuaternion.x = this.quaternion.x;
			this.collision.interpolatedQuaternion.y = this.quaternion.y;
			this.collision.interpolatedQuaternion.z = this.quaternion.z;
			this.collision.interpolatedQuaternion.w = this.quaternion.w;
