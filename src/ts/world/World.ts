...
export class World
{
	...
	public getMyUserData(): Object 
	{
		...
		let positionX = Math.sin(Math.random() * Math.PI * 2) * 1;
		let positionY = Math.sin(Math.random() * Math.PI * 2) * 1;
		// let positionX = 0;
		// let positionY = 0;
		// let rotation = Math.sin(Math.random() * Math.PI * 2) * Math.PI;
	...
	private createParamsGUI(scope: World): void
	{
		this.params = {
			...
			Sun_Elevation: 50,
			Sun_Rotation: 145,
			// Sync_Character_Enable: true,
			Sync_Vehicle_Enable: true,
		};
		...
		// settingsFolder.add(this.params, 'Sync_Character_Enable')
		// 	.onChange((enabled) => {
		// 		if (enabled) enableCharacterUpdate = true;
		// 		else enableCharacterUpdate = false;
		// 	})
		settingsFolder.add(this.params, 'Sync_Vehicle_Enable')
			.onChange((enabled) => {
				if (enabled) enableVehicleUpdate = true;
				else enableVehicleUpdate = false;
			})
